...
def trace_gen(n_db_records, chip_level_count_entry, query_index, query_row_opening, salp):
	...
	records_per_row = int(bits_per_row / record_width)
	print('DEBUG records_per_row: %d' % records_per_row)

	total_db_blocks = int(n_db_records / records_per_row)
	...
	num_db_blocks_per_sa = int(total_db_blocks / total_n_sa)
	print('DEBUG num_db_blocks_per_sa: %d' % num_db_blocks_per_sa)

	num_db_rows_per_sa = int(num_db_blocks_per_sa * record_height)
	...
	for SA in subArray_lst:
		for BA in bank_lst:
			for RA in rank_lst:
				for CH in chan_lst:
					for row_index in range(0,num_db_rows_per_sa):
						num_filter_read += 1


	# we estimate for each row latched into the row buffer, 10 additional reads are generated to simulate ALU time
	
	for i in range(int(num_filter_read * 10 * query_row_opening[query_index])):
		addr_seg_map = {
...
# salp_list = [1, 2, 4, 8, 16, 32, 64, 128] 
salp_list = [1]
...
### Q1, 2, 7, 8, 9, 10, 11: 138 * 3 / 512 --> 0.81, 
### Q3, 12, 13: 138 * 4 / 512 -> 1.07
### Q4, 5, 6: 138 * 2 / 512 -> 0.54
