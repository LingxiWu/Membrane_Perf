...
def trace_gen(chip_level_count_entry, db_block_to_sa_map, query_index, salp):
	...
	output_prefix = '/home/lw2ef/Documents/workspace/DRAM_DB/ramulator-master/SSB_trace/Sieve_Mode_One/Reads/'
	trace_file = output_prefix + q_s + '_Sieve_Mode_One_Reads.trace'
	...
	# make reads to emulate LISA mechanism
	LISA_list = []
	num_lisa_reads = int(lisa_reads_ratio[salp][query_index] * read_count)
	for idx in range(num_lisa_reads):
		db_block = random.choice(db_block_to_sa_map)
		addr_seg_map = {
			'Channel': db_block[0],
			'Rank': db_block[1],
			'Bank': db_block[2],
		...
		LISA_list.append(hex(int(addr_temp,2))+" R"+"\n")


	print('In trace_gen() -> Total number of read commands: %d' % read_count)
	print('In trace_gen() -> Total number of LISA read commands: %d' % len(LISA_list))
	# print('In trace_gen() -> Total number of write commands: %d' % write_count)
	print('In trace_gen() -> Total number of read commands: %d' % (read_count + len(LISA_list)))

	random.shuffle(read_list)
	# random.shuffle(write_list)
	for read in read_list:
		fo.write(read)
	for lisa_read in LISA_list:
		fo.write(lisa_read)
...
n_sa = 8 # n_sa=4 covers salp1 (2 banks) ~ salp64 (128 banks). for salp128, n_sa = 8
n_chan = 8
n_rank = 2
