...
def trace_gen(n_db_records, chip_level_count_entry, query_index, query_row_opening, salp):
	...
	trace_file = output_prefix + 'Q' + str(query_index+1) + '_salp_' + str(salp) +  '_Fulcrum_Mode_One_ALU.trace'
	...
	largest_attribute_len = 23 # lo revenue
	num_attributes_per_row = math.ceil(bits_per_row / largest_attribute_len)
	...
	print('DEBUG num rows to check per db block: %d' % query_row_opening[query_index])
	...
	read_alu_list = []
	for i in range(int(num_filter_read * 10)):
		addr_seg_map = {
...
# SSB WideTable attributes document: https://docs.google.com/spreadsheets/d/1XH2veCk0n_wfCnFGVNx9NVp4AJ6o6LRoNhhDAKdzce4/edit?pli=1#gid=1962010669
# a complete row major layout means each row stores one attributes. To support query, there are 18 attributes/rows
query_row_opening = [3, 3, 4, 2, 2, 2, 3, 3, 3, 3, 3, 4, 4]
