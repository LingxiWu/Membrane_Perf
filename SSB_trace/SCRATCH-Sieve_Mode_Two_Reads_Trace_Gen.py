...
# assign predicate blocks to 
def assignDBBlocksToSAs(n_db_records, db_record_bit_length_predicate, db_record_bit_length_payload, chip_level_count_entry, salp):
	...
	print('In assignDBBlocksToSAs() -> num_predicate_db_block_per_sa: %d' % num_predicate_db_block_per_sa)
	...
	num_payload_db_block_per_sa = math.floor(chip_level_count_entry[chip_levels.index("Row")] / db_record_bit_length_predicate)
	print('In assignDBBlocksToSAs() -> num_predicate_db_block_per_sa: %d' % num_payload_db_block_per_sa)
	...
	## record how many blocks each subarray has stored
	sa_block_counter = {} # {[CH,RA,BA,SA]:0, [CH,RA,BA,SA]: 0}
	for sa in subArray_lst:
		for ba in bank_lst:
			for ra in rank_lst:
				for ch in chan_lst:
					sa_addr = [ch,ra,ba,sa]
					sa_block_counter[tuple(sa_addr)] = 0
	# print('sa_block_counter: ',sa_block_counter) # DEBUG

	# prioritize assigning predicate db blocks to smart SAs
	PredicateToSAMapping = {}
	total_smart_db_block = math.floor(num_predicate_db_block_per_sa * salp * chip_level_count_entry[chip_levels.index("Channel")] * chip_level_count_entry[chip_levels.index("Rank")] * chip_level_count_entry[chip_levels.index("Bank")])
	# print('salp: %d, total_smart_db_block: %d' % (salp, total_smart_db_block)) # DEBUG
	if total_smart_db_block < total_n_predicate_blocks:
		PredicateToSAMapping = distributeToSmartSA(total_smart_db_block, chan_lst, rank_lst, bank_lst, subArray_lst, smart_sa_index, sa_block_counter)
		# second, for the remaining predicate db blocks
		PredicateDBBlockToDumbSaMapping = distributeToDumbSA(total_n_predicate_blocks,total_smart_db_block,chan_lst, rank_lst, bank_lst, subArray_lst, smart_sa_index, sa_block_counter)
		PredicateToSAMapping.update(PredicateDBBlockToDumbSaMapping)
	else:
		PredicateToSAMapping = distributeToSmartSA(total_n_predicate_blocks, chan_lst, rank_lst, bank_lst, subArray_lst, smart_sa_index, sa_block_counter)

	# next, we distribute payload blocks to any subarrays available
	PayloadToSAMapping = {}
	PayloadToSAMapping = distributePayload(total_n_payload_blocks, chan_lst, rank_lst, bank_lst, subArray_lst, sa_block_counter)

	return PredicateToSAMapping, PayloadToSAMapping


def distributeToSmartSA(total_smart_db_block, chan_lst, rank_lst, bank_lst, subArray_lst, smart_sa_index, sa_block_counter):
			...
			for BA in bank_lst:
				for RA in rank_lst:
					for CH in chan_lst:
						...
						sa_block_counter[tuple([CH,RA,BA,SA])] += 1
						i += 1

def distributeToDumbSA(total_n_predicate_blocks, total_smart_db_block, chan_lst, rank_lst, bank_lst, subArray_lst, smart_sa_index, sa_block_counter):
	...
	# distributing any blocks from 
	i = total_smart_db_block
	while i >= total_smart_db_block and i <= total_n_predicate_blocks:
			...
			for BA in bank_lst:
				for RA in rank_lst:
					for CH in chan_lst:
						...
						if sa_block_counter[tuple([CH,RA,BA,SA])] < 4:
							DBBlockToDumbSaMapping[i] = [CH,RA,BA,SA]
							sa_block_counter[tuple([CH,RA,BA,SA])] += 1
							i += 1	


def distributePayload(total_n_payload_blocks, chan_lst, rank_lst, bank_lst, subArray_lst, sa_block_counter):
	payloadToSAMapping = {}
	i = 0
...
def trace_gen(chip_level_count_entry, predicate_block_to_sa_map, payload_block_to_sa_map, query_index, salp):
	...
	output_prefix = '/home/lw2ef/Documents/workspace/DRAM_DB/ramulator-master/SSB_trace/Sieve_Mode_Two/'
	trace_file = output_prefix + q_s + '_Sieve_Mode_Two.trace'
	...
	print('len(passed_rec_ids_set): %d' % len(pased_rec_ids_set)) # DEBUG
...
predicate_attribute_bit_length = [[3,4,6], [7,4,6], [6,3,4,6],
						[5,3], [10,3], [10,3],
						[3,3,3], [5,5,3], [8,8,3],
						[8,7,8], [3,3,3], [3,3,3,3],
						[3,5,3,5]]
...
predicate_block_to_sa_map, payload_block_to_sa_map = assignDBBlocksToSAs(n_db_records, db_record_bit_length_predicate, db_record_bit_length_payload, chip_level_count_entry, salp)
