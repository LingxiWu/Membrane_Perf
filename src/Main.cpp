...
void run_dramtrace(const Config& configs, Memory<T, Controller>& memory, const char* tracename) {
    ...
    while (!end || memory.pending_requests()){
        if (!end && !stall){
            end = !trace.get_dramtrace_request(addr, type); // getting the address one by one from the trace file 
            // printf("Main.cpp run_dramtrace(): addr: %ld\n",addr);
        }

        if (!end){
            req.addr = addr;
            req.type = type;
            stall = !memory.send(req); // send memory request 
            if (!stall){
    ...
    memory.finish();
    printf("Main.cpp. Final clks: %d, Stats::curTick: %d \n", clks, Stats::curTick);
...
void start_run(const Config& configs, T* spec, const vector<const char*>& files) {
  ...
  // printf("Main::start_run n_chan: %d, n_rank: %d\n",C,R);
