...
public:
          ...
          addr_bits(int(T::Level::MAX))
    {
        ...
        // printf("Memory.h Memory(): channel_width: %d\n",spec->channel_width); // spec is specified in SALP.h
        

        for (unsigned int lev = 0; lev < addr_bits.size(); lev++) {
          addr_bits[lev] = calc_log2(sz[lev]);
            max_address *= sz[lev];
            // printf("Memory.h Memory(): sz[%d] %d\n",lev,sz[lev]);
        }

        // printf("Memory.h Memory(): max_address: %ld\n",max_address);

        // column index is cut short because of the prefetch. each prefetch grabs prefetch_size (8) columns, therefore fewer addressable columns
    ...
    {
        req.addr_vec.resize(addr_bits.size()); // addr_bits(int(T::Level::MAX))

        for(int i=0;i<addr_bits.size();i++){
            // printf("Memory::send(): addr_bits[%d]: %d\n",i,addr_bits[i]);
        }
        
        // printf("Memory::send(): addr_bits.size(): %zu\n",addr_bits.size());
        
        long addr = req.addr;
        int coreid = req.coreid;

        // printf("Memory::send(): req.addr: %ld\n", req.addr);

        // Each transaction size is 2^tx_bits, so first clear the lowest tx_bits bits
        clear_lower_bits(addr, tx_bits);

        // printf("Memory::send(): tx_bits: %d\n", tx_bits); // 6-bit
        // printf("Memory::send(): after clear_lower_bits: %ld\n",addr);

        if (use_mapping_file){
            apply_mapping(addr, req.addr_vec);
            // printf("Memory::send(): use_mapping_file: %d\n",use_mapping_file); // no special mapping
        }
        else {
            switch(int(type)){
                case int(Type::ChRaBaRoCo):
                    // printf("Memory::send(): mapping Type::ChRaBaRoCo\n");
                    for (int i = addr_bits.size() - 1; i >= 0; i--)
                        req.addr_vec[i] = slice_lower_bits(addr, addr_bits[i]);
                    break;
                case int(Type::RoBaRaCoCh):
                    // enum class Level : int {Channel, Rank, Bank, SubArray, Row, Column, MAX};
                    // printf("Memory::send(): mapping Type::RoBaRaCoCh\n"); // default mapping
                    req.addr_vec[0] = slice_lower_bits(addr, addr_bits[0]); // chan
                    req.addr_vec[addr_bits.size() - 1] = slice_lower_bits(addr, addr_bits[addr_bits.size() - 1]); // col
                    for (int i = 1; i <= int(T::Level::Row); i++) // rank, bank, subarray, row
                        req.addr_vec[i] = slice_lower_bits(addr, addr_bits[i]);
                    break;
                default:
                    assert(false);
...
private:
    ...
    int slice_lower_bits(long& addr, int bits)
    {
        ...
        addr >>= bits;
        // printf("Memory::slice_lower_bits(): n_bits %d\n",bits);
        // printf("Memory::slice_lower_bits(): addr after slice %d\n",lbits);
